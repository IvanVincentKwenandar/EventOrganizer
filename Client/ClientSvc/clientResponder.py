...
db = mysql.connector.connect(host="ClientSQL", user="root", password="root",database="clientdb")
...
def getClient(id):
    ...
    if HTTPRequest.method == 'GET':
        ...
        clientInfo = json.loads(HTTPRequest.data)
        sql = "UPDATE clientinfo SET namaClient = %s, emailClient = %s, passwordClient = %s, noTelpClient = %s, alamatClient = %s"
        cursor.execute(sql,[clientInfo['namaClient'],
                            clientInfo['userClient'],
                            clientInfo['passwordClient'],
                            clientInfo['noTelpClient'],
